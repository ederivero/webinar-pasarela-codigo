...
const {
  Storage
} = require("@google-cloud/storage"); // Inicializo mi objeto de Firebase para poner conectarme con mi Bucket
...
const storage = new Storage(credenciales); // se crea la variable bucket que se usa como referencia al link del storage
...
const subirArchivo = archivo => {
  return new Promise((resolve, reject) => {
    ...
    const nuevoNombre = `${archivo.originalname}_${Date.now()}`; // comenzamos a cargar nuestro archivo con el nuevo nombre pero aun no se sube a Firebase

    const fileUpload = bucket.file(nuevoNombre); // agregamos configuracion adicional de nuestro archivo a subir como su metadata
    ...
    blobStream.on("finish", () => {
      ...
      }).then(link => resolve(link)).catch(error => reject(`Error al devolver el link: ${error}`));
